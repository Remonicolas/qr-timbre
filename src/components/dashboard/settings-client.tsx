    ...
    <div className="space-y-5">
      ...
      <SectionCard title="Perfil" icon={User}>
        <form onSubmit={handleSubmit(onProfileSave)} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Teléfono personal</label>
            <input {...register('phone')} type="tel" placeholder="+54 11 1234-5678" className={inputClass()} />
          </div>
          <button type="submit" disabled={isSubmitting} className="flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground hover:opacity-90 transition-all disabled:opacity-50">
      ...
      {/* Push Notifications */}
      <SectionCard title="Notificaciones Push" icon={Bell}>
        <p className="text-sm text-muted-foreground">
          Recibí alertas en tu celular o computadora aunque tengas el navegador cerrado.
        </p>
        ...
        {isSubscribed || pushStatus === 'granted' ? (
          <div className="flex items-center gap-3 rounded-xl bg-green-500/10 border border-green-500/20 px-4 py-3">
            ...
            <div>
              <p className="text-sm font-semibold text-green-600">Notificaciones activas</p>
              <p className="text-xs text-muted-foreground">Recibirás alertas cuando toquen tu timbre</p>
            </div>
          </div>
        ) : pushStatus === 'denied' ? (
          <div className="flex items-center gap-3 rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-600">Notificaciones bloqueadas</p>
              <p className="text-xs text-muted-foreground">Habilitá los permisos en tu navegador manualmente</p>
            </div>
          </div>
        ) : (
          <button
          ...
          >
            {pushStatus === 'subscribing' ? <Loader2 size={16} className="animate-spin" /> : <Bell size={16} />}
      ...
      <SectionCard title="Seguridad" icon={Shield}>
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between items-center">
            <span className="text-muted-foreground">Propiedades permitidas</span>
            <span className="font-semibold">{profile.max_properties}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-muted-foreground">Miembro desde</span>
            <span className="font-semibold">{new Date(profile.created_at).toLocaleDateString('es-AR')}</span>
          </div>
        </div>
      </SectionCard>
    </div>
