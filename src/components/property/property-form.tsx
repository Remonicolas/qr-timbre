...
export function PropertyForm({ property }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    const url = isEditing
      ? API_ROUTES.property(property.id)
    ...
    toast.success(
      isEditing ? 'Propiedad actualizada' : 'Propiedad creada exitosamente'
    )
  ...
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <h3 className="font-display font-bold text-base border-b border-border pb-2 mb-4">
      {children}
    </h3>
  )
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
        ...
        <div>
          ...
          <input
            {...register('name')}
            placeholder="Ej: Casa principal, Oficina centro"
          ...
          {Object.entries(PROPERTY_STATUS_LABELS).map(([value, label]) => {
            ...
            return (
              <label
                key={value}
                className={cn(
                  'flex items-center gap-2 cursor-pointer rounded-xl border-2 px-3 py-2.5 text-sm font-medium transition-all',
                  current === value
      ...
      {/* NOTIFICATIONS (🔥 FIREBASE REAL SWITCH) */}
      <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
        ...
        <label className="flex items-center justify-between cursor-pointer mt-4">
          ...
          <input
            {...register('is_building_mode')}
            type="checkbox"
            className="sr-only peer"
          />
          <div className="w-11 h-6 bg-border rounded-full peer peer-checked:bg-primary relative after:absolute after:top-0.5 after:left-0.5 after:h-5 after:w-5 after:bg-white after:rounded-full after:transition-all peer-checked:after:translate-x-5" />
        </label>

        <div>
          <label className="block text-sm font-medium mb-1.5">
            Tiempo entre timbrazos
          </label>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="flex-1 rounded-xl bg-primary text-white px-4 py-3"
        >
          {isSubmitting && (
