...
export const metadata: Metadata = {
  title: `${APP_CONFIG.name} - El timbre inteligente sin revelar tu número`,
...
const features = [
  {
    icon: '🔔',
    title: 'Timbre Digital',
    description: 'Tus visitantes escanean el QR y te avisan. Sin revelar tu teléfono ni tu número.',
  },
  {
    icon: '📦',
    title: 'Categorías de Visita',
    description: 'Delivery, invitados, correo, emergencia. Sabés quién llama antes de bajar.',
  },
  {
    icon: '📲',
    title: 'Notificaciones Instantáneas',
    description: 'Recibí alertas en tiempo real aunque tengas la app cerrada.',
  },
  {
    icon: '🏢',
    title: 'Modo Edificio',
    description: 'Un QR, múltiples departamentos. Perfecto para edificios y condominios.',
  },
  {
    icon: '💬',
    title: 'Respuestas Rápidas',
    description: '"Ya bajo", "Dejá el paquete" y más. Respondé en un toque.',
  },
  {
    icon: '🔒',
    title: '100% Privado',
    description: 'Tu número nunca se comparte. Seguridad total para vos y tu familia.',
  },
]
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-8">
              <Link href="#funciones" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                Funciones
              </Link>
      ...
      <section className="relative pt-32 pb-20 px-4 overflow-hidden">
        ...
        <div className="mt-20 mx-auto max-w-sm">
          <div className="rounded-3xl border border-border bg-card shadow-xl shadow-black/10 p-6 text-center" style={{ animation: 'float 3s ease-in-out infinite' }}>
            <div className="text-6xl mb-4">🔔</div>
            <div className="font-display text-lg font-bold mb-1">Casa Principal</div>
            <div className="text-sm text-muted-foreground mb-6">Escaneá para llamar</div>
            <div className="mx-auto w-32 h-32 bg-foreground/5 rounded-2xl flex items-center justify-center border border-border">
              <div className="grid grid-cols-7 gap-0.5 w-24 h-24 opacity-80">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature) => (
              <div
                key={feature.title}
                className="rounded-2xl border border-border bg-card p-6 hover:border-primary/30 hover:shadow-lg transition-all group"
              >
      ...
      <section className="py-24 px-4 bg-secondary/30">
        <div className="mx-auto max-w-5xl text-center">
          <h2 className="font-display text-4xl font-extrabold mb-4">
            Tu estado, tu control
          </h2>
          <p className="text-muted-foreground text-lg mb-12">
            Tus visitantes ven tu estado antes de tocar el timbre.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            {statuses.map((s) => (
              <div
                key={s.label}
                className="rounded-2xl border border-border bg-card px-6 py-3 font-medium text-sm"
              >
                {s.emoji} {s.label}
              </div>
            ))}
